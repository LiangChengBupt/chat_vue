<template>
  <div id="message" v-scroll-bottom="privateSessions">
    <div v-if="currentSession && currentSession.username != '群聊'">
      ...
      <ul>
        <li v-for="entry in privateSessions[user.username + '#' + currentSession.username]" :key="entry.id">
        <!-- <li v-for="entry in privateSessions['私聊占位']" :key="entry.id"> -->
          <p class="time">
...
export default {
  ...
  computed: mapState(["sessions","privateSessions", "currentSession", "errorImgUrl"]),
  filters: {
    time(date) {
      if (date) {
        date = new Date(date);
